...
{
	global $con;
    $query="select category.id,category.name,category.image,count(product.id) as count from product join category on category.id = category_id group by category.name order by count desc limit 3 ";
    $result=mysqli_query($con,$query);
    return $result;
}
//----------------------------
...
{
    global $con;
    $query="select category.id,category.name,category.image,count(product.id) as count from product join category on category.id = category_id group by category.name order by count desc limit 4 ";
    $result=mysqli_query($con,$query);
    return $result;
}
//--------------------------
...
function register($name,$role,$card_id,$email,$pass)
{
	...
	echo '<script>alert("Register Faild")</script>';
	}
}
//----------------------------
...
function confirm_purchase($order_date,$customer_id,$total,$pick,$delivery_type,$delivery_location,$address,$delivery_date)
{
	...
	$_SESSION['cart']=array();
	echo '<script>alert("Request Confirmed Thank You")</script>';
    }
	echo '<script>window.location="customer.php?id='.$_SESSION["customer_id"].'"</script>';
